...
\chapter{Problem grobe poti}\label{cha:teoreticne_osnove}
...
%Sedaj ko smo seznanjeni s splošnim delovanjem krmilnega sistema avtonomnega vozila se lahko osredotočimo na problem. Navigacija avtonomnega vozila pomeni usmerjanje vozila tako, da se izogiba oviram v njegovi okolici. V sistemu ROS je navigacija realizirana tako, da se najprej kartografira okolico vozila pred končno uporabo robota. To se stori tako, da z ročnim upravljanjem robota na podlagi senzorskih podatkov (lidar) in kartografskih algoritmov, kot je SLAM (ang. \textit{Simultaneous Localization And Mapping}) izrišemo taki imenovani zemljevid vrednosti (ang. \textit{Costmap}). To so  Navigacija v okolju ROS se izvaja na podlagi zemljevidov uteži (Angl.: \textit{Costmap}). To so dvodimenzionalne mreže pravokotne oblike, kjer so ovire predstavljene z utežmi. Razdelki v zemljevidu uteži imajo tako vrednost 0 v primeru odsotnosti ovire ter vrednosti 100 v primeru ovire.
%
...
Do teh zemljevidov pridemo z uporabo kartografskih algoritmov kot je na primer \textit{Simultaneous Localization And Mapping} - SLAM. Mrežo zasedenosti okolja robota dobimo na podlagi izhodnih informacij senzorjev (lidar) in sicer tako, da zaženemo kartografski algoritem in nato ročno ali avtomatsko vodimo robota po okolju tako, da zajamemo celotno okolico. Ko imamo zajeto celotno območje lahko izvozimo mrežo v obliki slike. Kartografske algoritme nam nudi okolje ROS.
